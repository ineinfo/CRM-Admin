...
export default function UserEditForm({ metadata, currentUser }) {
  const { user: authUser } = useAuthContext(); // Renamed to authUser for clarity
  const router = useRouter();
  const { products: roles } = UsegetRoles();
  const { enqueueSnackbar } = useSnackbar();

  // Memoizing user data to prevent losing it on re-renders
  ...
  // Reset form data when user data is updated
  useEffect(() => {
    if (user) {
      reset(defaultValues);
    }
